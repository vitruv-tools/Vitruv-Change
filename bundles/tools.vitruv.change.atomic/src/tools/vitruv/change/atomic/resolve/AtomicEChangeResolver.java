...
class AtomicEChangeResolver<Source, Target> {
	private Function<Source, Target> resolver;

	private AtomicEChangeResolver(Function<Source, Target> resolver) {
		checkArgument(resolver != null, "resolver must not be null");
		this.resolver = resolver;
	}

	static <Source, Target> EChange<Target> resolveChange(EChange<Source> eChange, Function<Source, Target> resolver) {
		return new AtomicEChangeResolver<>(resolver).resolve(eChange);
	}
	...
	private <F extends EStructuralFeature> void resolveFeatureEChange(FeatureEChange<Source, ?> sourceChange,
		...
		checkArgument(sourceChange.getAffectedFeature() != null, "change %s must have an affected feature",
				sourceChange);
		targetChange.setAffectedElement(resolver.apply(sourceChange.getAffectedElement()));
	...
	private void resolve(EObjectAddedEChange<Source> sourceChange, EObjectAddedEChange<Target> targetChange) {
		if (sourceChange.getNewValue() != null) {
			targetChange.setNewValue(resolver.apply(sourceChange.getNewValue()));
	...
	private void resolve(EObjectSubtractedEChange<Source> sourceChange, EObjectSubtractedEChange<Target> targetChange) {
		if (sourceChange.getOldValue() != null) {
			targetChange.setOldValue(resolver.apply(sourceChange.getOldValue()));
	...
	private void resolve(RootEChange<Source> sourceChange, RootEChange<Target> targetChange) {
		// TODO: how to handle RootEChange?
		// Get resource where the root object will be inserted / removed.
//		targetChange.resource = uuidResolver.getResource(URI.createURI(change.uri))
	}
