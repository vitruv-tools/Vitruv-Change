...
class ResourceContainmentMatcher extends TypeSafeMatcher<Resource> {
  ...
  public void describeMismatchSafely(final Resource item, final Description mismatchDescription) {
    if ((!this.exists)) {
      ...
      if (_isEmpty) {
        ...
        if (_greaterThan) {
          ModelPrinting.appendModelValue(mismatchDescription, item).appendText(" contained ")
              .appendValue(Integer.valueOf(item.getContents().size()))
          ...
          ModelPrinting.appendModelValue(
              mismatchDescription.appendText(System.lineSeparator()).appendText("    in the resource at "),
              item.getURI());
        }
      }
    }
  ...
  public boolean matchesSafely(final Resource item) {
    this.exists = item.getResourceSet().getURIConverter().exists(item.getURI(), Map.of());
    return ((this.exists && (item.getContents().size() == 1))
        && this.delegateMatcher.matches(item.getContents().get(0)));
  }
