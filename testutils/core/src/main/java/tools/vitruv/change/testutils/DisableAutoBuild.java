...
/**
 * Disables automatic building in an Eclipse workspace during the test, and sets
 * it back
 * to its old value afterwards. Disables automatic building both before all
 * tests and
 * before each test. That means that @BeforeAll
 */
public class DisableAutoBuild implements BeforeAllCallback, BeforeEachCallback {
  private static class EnableAutoBuildGuard implements AutoCloseable {
    @Override
