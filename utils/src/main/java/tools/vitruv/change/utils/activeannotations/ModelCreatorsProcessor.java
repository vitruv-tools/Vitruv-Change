...
public class ModelCreatorsProcessor extends AbstractClassProcessor {
    ...
    public void doTransform(MutableClassDeclaration annotatedClass, TransformationContext context) {
        ...
        if (annotation == null) {
            context.addError(annotatedClass, "Cannot find ModelCreators annotation on " + annotatedClass.getSimpleName());
        }
        if (context.getProblems(annotatedClass).stream().anyMatch(p -> p.getSeverity() == Problem.Severity.ERROR)) {
            return;
        }
        ...
        Preconditions.checkArgument(rawReplacements.length % 2 == 0,
            "The replace array must have an even number of elements!");
        ...
        StreamSupport.stream(factory.getDeclaredResolvedMethods().spliterator(), false)
            ...
            .filter(m -> m.getSimpleName().startsWith("create") && !m.getParameters().iterator().hasNext())
            .forEach(createMethod -> {
                String baseName = removePrefix(removePrefix(createMethod.getSimpleName(), "create"), stripPrefix);
                String targetName = prefix + toFirstUpper(replacements.getOrDefault(baseName, baseName));
                TypeReference returnType = createMethod.getReturnType();
                annotatedClass.addMethod(targetName, m -> {
                    m.setStatic(createStatic);
                    m.setReturnType(returnType);
                    m.setBody(ctx -> "return " + ctx.toJavaCode(factory) + ".eINSTANCE." + createMethod.getSimpleName() + "();");
                    context.setPrimarySourceElement(m, annotatedClass);
                });
            });

        annotatedClass.addMethod("_getClassifier", m -> {
            ...
            m.setBody(ctx -> {
                String preconditions = ctx.toJavaCode(context.newTypeReference(Preconditions.class));
                String factoryCode = ctx.toJavaCode(factory);
                return "return " + preconditions + ".checkNotNull(\n"
                    + "\t" + factoryCode + ".eINSTANCE.getEPackage().getEClassifier(classifierName),\n"
                    + "\t\"There is no classifier called '%s' in '%s'!\", classifierName, "
                    + factoryCode + ".eINSTANCE.getEPackage().getName()\n"
                    + ");";
            });
        });

        annotatedClass.addMethod("classifier", m -> {
            m.setReturnType(context.newTypeReference(EClassifier.class));
            m.addParameter("classifierName", context.newTypeReference(String.class));
            m.setBody(ctx -> "return _getClassifier(classifierName);");
        });

        annotatedClass.addMethod("_createInstance", m -> {
            ...
            m.setBody(ctx -> {
                ...
                String factoryCode = ctx.toJavaCode(factory);
                return eClassifier + " requestedClassifier = _getClassifier(className);\n"
                    + preconditions + ".checkArgument(\n"
                    + "\trequestedClassifier instanceof " + eClass + ",\n"
                    + "\t\"%s is not an EClass and can thus not be instantiated!\", className\n"
                    + ");\n"
                    + "return " + factoryCode + ".eINSTANCE.create((" + eClass + ") requestedClassifier);";
            });
        });

        annotatedClass.addMethod("create", m -> {
            m.setReturnType(context.newTypeReference(EObject.class));
            m.addParameter("className", context.newTypeReference(String.class));
        ...
        annotatedClass.addMethod("create", m -> {
            MutableTypeParameterDeclaration typeParam = m.addTypeParameter("M",
                context.newTypeReference(EObject.class));
            m.setReturnType(context.newSelfTypeReference(typeParam));
            m.addParameter("clazz",
                context.newTypeReference(Class.class,
        ...
        var newEObjectClass = context.findClass(annotatedClass.getQualifiedName() + "." + NEW_EOBJECT_CONVERTER);
        newEObjectClass.setImplementedInterfaces(List.of(context.newTypeReference(ArgumentConverter.class)));
        newEObjectClass.addMethod("convert", m -> {
            m.setReturnType(context.newTypeReference(EObject.class));
            ...
            m.setBody(ctx -> {
                String exType = ctx.toJavaCode(context.newTypeReference(ArgumentConversionException.class));
                return "try {\n"
                    + "\treturn _createInstance((String) source);\n"
                    + "} catch (IllegalArgumentException | NullPointerException e) {\n"
                    + "\tthrow new " + exType + "(e.getMessage(), e);\n"
                    + "}";
            });
        });

        var classifierClass = context.findClass(annotatedClass.getQualifiedName() + "." + CLASSIFIER_CONVERTER);
        classifierClass.setImplementedInterfaces(List.of(context.newTypeReference(ArgumentConverter.class)));
        classifierClass.addMethod("convert", m -> {
            m.setReturnType(context.newTypeReference(EObject.class));
            m.addParameter("source", context.newTypeReference(Object.class));
            m.addParameter("parameterContext", context.newTypeReference(ParameterContext.class));
            m.setExceptions(context.newTypeReference(ArgumentConversionException.class));
