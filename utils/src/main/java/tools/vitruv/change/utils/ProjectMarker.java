...
/**
 * Provides functionality to mark file system folders as project root folders with a marker file and
 * identify such a project folder from any given subfolder.
 */
public final class ProjectMarker {
  ...
  /**
   * Marks the folder with the given path as a project root folder by placing a marker file to be
   * identified by {@link #getProjectRootFolder(Path)}.
   *
   * @param projectRootFolder the path of the root folder to mark, must not be <code>null</code>
   * @throws IOException if something went wrong when creating the marker file (see {@link
   *     Files#createFile(Path, java.nio.file.attribute.FileAttribute...)}.
   */
  ...
  /**
   * Returns the path to the project in which the element with the given path is contained. This
   ...
   * #markAsProjectRootFolder(Path)}. Otherwise, an {@link IllegalStateException} will be thrown.
   *
  ...
  public static Path getProjectRootFolder(Path containedElementPath) {
    ...
    while (!Files.exists(potentialProjectPath.resolve(TEST_PROJECT_MARKER_FILE_NAME))) {
      ...
      checkState(
          potentialProjectPath != null, "No project folder for %s found", containedElementPath);
    }
