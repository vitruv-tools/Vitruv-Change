...
public class CorrespondenceTest {
  ...
  public void testReloadingCorrespondencesFromPersistence() {
    ...
    Set<EObject> _allEObjects = new HashSet<>(_leftEObjects);
    _allEObjects.addAll(_rightEObjects);
    Assertions.assertEquals(Set.<EObject>of(repo2, pkg2), _allEObjects);
  ...
  private Repository createPcmRepositoryWithInterfaceAndComponent(final URI persistenceURI) {
    ...
    _function.accept(_createRepository);
  ...
  private UPackage createUmlPackageWithInterfaceAndClass(final URI persistenceURI) {
    ...
    final Consumer<UPackage> _function = (UPackage it) -> {
    ...
    _function.accept(_createUPackage);
    return _createUPackage;
  }
