...
public class ObjectResolutionUtil {
  ...
  /**
   * Returns a hierarchic URI fragment for the given {@link EObject}. Its use case
   * is to resolve the
   * same object in different instances of the same resource loaded into different
   * resource sets.
   * It is essential that the resources must be in the exact same state, and it is
   * <b>not</b>
   * guaranteed that using the returned fragment objects can always be resolved,
   * but at least
   * it gives the chance to do so whenever the ordinary URI delivered by EMF is
   * not sufficient.
   * So use this method with care!
   * 
   * <p>
   * Metamodels may use XMI IDs, which are used by EMF as the URI fragment of
   * objects
   ...
   * across different resource sets containing a loaded instance of the same
   * resource.
   * The URI fragment generated by this method depends on the fragment given by
   * {@link EcoreUtil#getRelativeURIFragmentPath} and thus does circumvent the
   * problem of XMI IDs.
   * 
   * <p>
   * In consequence, for an object <code>objectInResourceSet1</code>
   * contained in a {@link ResourceSet} <code>resourceSet1</code>, it should be
   * possible
